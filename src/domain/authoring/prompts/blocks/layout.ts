...
- \`::: diagram {"kind":"flow"|"architecture"|"sequence"|"hierarchy"|"timeline","direction":"down"|"right","nodes":[{"id":"a","label":"Request","note":"optional second line","accent":1-4,"group":"g1"}],"edges":[{"from":"a","to":"b","label":"yes"|null,"style":"solid"|"dashed","head":"arrow"|"none"}],"groups":[{"id":"g1","label":"Backend","parent":"g0"}],"title":"..."|null,"caption":"..."|null}\` — no content, closed on the next line. You declare MEANING ONLY: never a coordinate, a width or a position — the editor places every box. 1-24 nodes, at most 40 edges, ids unique, and every "from"/"to" MUST name a declared node. "note", "accent" and "group" are optional; a "group" must appear in "groups" and draws a labelled band (architecture only, in practice). A box's bullets fold into "note" (≤160 chars, comma-separated), never dropped or stacked onto "label". A group's "parent" is another group's id and draws its band INSIDE that one — use it whenever the source shows a box within a box (a subscription holding a cluster holding namespaces); omit it at the top level, never make two groups each other's parent, and nest at most 4 deep. E.g. subscription→cluster→namespace: \`{"id":"g1","parent":"g0"}\`, \`{"id":"g2","parent":"g1"}\` — each names only its direct parent. Kinds: "flow" = a process, which MAY loop back; "architecture" = named parts of a system, usually grouped; "sequence" = participants exchanging messages, one edge per message IN ORDER, needs 2+ nodes and 1+ edge; "hierarchy" = a TREE, exactly one root and exactly one parent per other node, no cycles; "timeline" = phases in the order of "nodes" and NO edges at all. ONLY draw relations stated in the user's request or already present in the document — NEVER invent an architecture, a team or a process you were not given.`;
